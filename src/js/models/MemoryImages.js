...
class MemoryImages {
  ...
  displayMemoryImage(id, copy) {
    const findClickedElement = this.clonedMemoryListToGetPairsOfImages
      .filter(image => image.id === id)
      .find(image => image.isCopy === JSON.parse(copy));
    findClickedElement.isVisible = true;
    this.activeCards.push(findClickedElement);
  }
  ...
  checkIfImagesMatch() {
    ...
    if (checkIfImagesAreTheSame) {
      ...
      this.activeCards.length = 0;
      this.clonedMemoryListToGetPairsOfImages.map(
